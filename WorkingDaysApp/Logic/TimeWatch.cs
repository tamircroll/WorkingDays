...
namespace WorkingDaysApp.Logic
{
    ...
    public class TimeWatch
    {
        ...
        public void SetTime(int i_MonthDay, eColumn i_Column, string i_TimeToSet)
        {
            ...
            string[] lineArr = fileLines[lineToEdit].Split(sr_RowSeparator);
        ...
        {
            List<string> fileLines = FilesHandler.GetFileLines(m_ChosenYearInt, m_ChosenMonthInt);
            string[] dayDataToSetArr = fileLines[i_RowToSet].Split(sr_RowSeparator);
...
Set current time instead?",
        ...
        public string[] GetSummaryArr()
        {
            ...
            {
                string[] dayArr = day.Split(sr_RowSeparator);
        ...
        private string dayDataArrTostring(string[] i_DayDataToSetArr)
        {
            ...
            {
                i_DayDataToSetArr[i] = i_DayDataToSetArr[i].Replace(sr_RowSeparator.ToString(), sr_DashReplacer);
            }
            return String.Format(ROW_FORMAT, i_DayDataToSetArr[0], i_DayDataToSetArr[1], i_DayDataToSetArr[2],
                i_DayDataToSetArr[3],
                i_DayDataToSetArr[4], i_DayDataToSetArr[5], i_DayDataToSetArr[6], sr_RowSeparator);
        }
        ...
        private float getDailyWorkScope(string[] i_DayArr)
        {
